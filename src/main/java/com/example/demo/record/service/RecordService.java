...
public class RecordService {
	...
	public Map<String, String> getTodayFoodRecord(int userId, Date date) {
		List<FoodRecordsDto> foodRecord = getFoodRecords(userId, date);
		Map<String, String> todayFoodRecord = new HashMap<>(Map.of(
				"breakfast", "미입력",
				"lunch", "미입력",
				"dinner", "미입력",
				"snack", "미입력",
		...
		int totalKcal = 0;
		for (FoodRecordsDto record : foodRecord) {
			todayFoodRecord.put(record.getMealType(), record.getKcal() + "Kcal");
			totalKcal += record.getKcal();
		}
		todayFoodRecord.put("total", String.valueOf(totalKcal) + "Kcal");

		return todayFoodRecord;
	}
}
